...
const AdminOrderReviews = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      console.error("AdminOrderReviews error:", error);
  ...
  // compute overall averages like vendor view
    ...
    return (
      <div className="space-y-1">
        <div className="flex justify-between text-[11px] text-gray-600">
          <span>{label}</span>
          <span className="font-semibold">{value?.toFixed(1) || "0.0"}/5</span>
        </div>
  ...
  // Render media similar to vendor view (supports single media object or array; shows first)
  const renderMediaSection = (review) => {
    if (!review || !review.media) return null;

    // support array or single object
    const mediaList = Array.isArray(review.media) ? review.media : [review.media];
    if (mediaList.length === 0) return null;

    // prefer first for inline display (vendor shows single media)
    const media = mediaList[0];
    const url = media.url || media.secure_url || media.path || "";
    if (!url) return null;

    const mime = (media.mimeType || media.mimetype || "").toLowerCase();
    const isImage = mime.startsWith("image/") || /\.(jpe?g|png|gif|webp|bmp)$/i.test(url);
    const isVideo = mime.startsWith("video/") || /\.(mp4|webm|ogg|mov)$/i.test(url);

    const name = media.originalName || media.public_id || "Attached file";

    return (
      <div className="mt-3 bg-white border border-blue-50 rounded-xl p-3 shadow-sm">
        <p className="text-[11px] font-semibold text-gray-700 mb-2">Review Media</p>
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-white via-[#e8f3fc] to-[#d9e9fb]">
      ...
      <main className="flex-1 flex justify-center px-4 py-10 md:ml-34 transition-all duration-300">
        <div className="w-full max-w-6xl mx-auto">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500 mt-0.5">
                      ...
                      <span className="font-semibold">
                        {overallStats.count} review{overallStats.count > 1 ? "s" : ""}
                      </span>
                    </p>
                  </div>
                </div>
          ...
          {/* Main grid: left list, right detail (matches vendor layout) */}
          <div className="grid lg:grid-cols-[1.2fr_1.8fr] gap-4">
            ...
            <div className="bg-white/95 rounded-2xl border border-blue-50 shadow-[0_12px_30px_rgba(36,121,194,0.15)] p-4">
              <h2 className="text-sm font-semibold text-[#2479C2] mb-3">Reviewed Orders</h2>
              ...
              {loading ? (
                ...
                <div className="max-h-[70vh] overflow-y-auto space-y-2">
                  {orders.map((o) => {
                    ...
                    const rv = o.review || {};
                    const avgForOrder =
                      (rv.ratingManufacturing + rv.ratingDelivery + rv.ratingHandling) / 3 || 0;
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="font-semibold truncate max-w-[180px]">{o.title || "Order"}</p>
                              <p className={`text-[11px] truncate max-w-[180px] ${isActive ? "text-blue-50/90" : "text-gray-500"}`}>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            <span className={`text-[10px] ${isActive ? "text-blue-50/80" : "text-gray-400"}`}>
                  ...
                  <div className="flex justify-between items-start mb-3 border-b border-blue-100 pb-3">
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-gray-500 flex items-center gap-2">
                        ...
                        <span>
                          {selected.user?.name || "User"} • {selected.user?.email || "No email"}
                        </span>
                      </p>
                      <p className="text-[11px] text-gray-400 flex items-center gap-2">
                        <FaCalendarAlt className="text-blue-300" />
                        <span>
                          Reviewed on {formatDate(selected.review?.createdAt || selected.updatedAt)}
                        </span>
                      </p>
                    </div>
                    <button
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 flex-1">
                      ...
                      <div className="lg:w-1/2 space-y-3">
                        <div className="bg-gradient-to-br from-amber-50 via-white to-yellow-50 border border-amber-100 rounded-xl p-3 shadow-sm">
                          <p className="text-[11px] font-semibold text-amber-700 mb-1">Overall Impression</p>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <p className="text-[10px] text-gray-500">Manufacturing • Delivery • Handling</p>
                            </div>
                          </div>
                        </div>

                        <div className="bg-white border border-blue-50 rounded-xl p-3 shadow-sm space-y-2">
                          {renderRatingBar("Manufacturing Quality", selected.review.ratingManufacturing)}
                          {renderRatingBar("Delivery Experience", selected.review.ratingDelivery)}
                          {renderRatingBar("Handling & Packaging", selected.review.ratingHandling)}
                        </div>
                      </div>

                      {/* Comment + media */}
                      <div className="lg:w-1/2 flex flex-col">
                        <p className="text-[11px] font-semibold text-gray-600 mb-2">Client Feedback</p>
                        <div className="relative bg-gradient-to-br from-slate-50 via-white to-blue-50 border border-blue-100 rounded-2xl p-3 shadow-sm">
                          ...
                          <p className="mt-3 text-[10px] text-gray-400 italic">
                            This feedback is visible to admin and used to improve service quality.
                          </p>
                        </div>

                        {/* Media section (matches vendor) */}
                        {renderMediaSection(selected.review)}
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500">No review details found for this order.</p>
                  )}
                </>
              ) : (
                <div className="flex items-center justify-center flex-1">
                  <p className="text-xs text-gray-500">Select an order from the left to view its review details.</p>
                </div>
              )}
            </div>
          </div>
