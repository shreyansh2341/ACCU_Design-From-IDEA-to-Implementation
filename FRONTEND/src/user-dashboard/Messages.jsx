...
const Messages = () => {
  ...
  const [activeOrders, setActiveOrders] = useState([]); // now just "all orders for chat"
  const [selectedOrder, setSelectedOrder] = useState(null);

  const [messages, setMessages] = useState([]);
  const [messageText, setMessageText] = useState("");

  // NEW: pagination state for orders list
  ...
  const fetchMyOrders = async () => {
    try {
      ...
      // ✅ now include ALL orders (completed + cancelled also visible)
      setActiveOrders(list);
      // reset to first page when list updates
      setCurrentPage(1);

      if (
        selectedOrder &&
        !list.find((o) => o._id === selectedOrder._id)
      ) {
        // order was deleted
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      // 🔁 15s when active, 20s when tab is hidden
  ...
  const totalPages =
    Math.ceil(activeOrders.length / pageSize) || 1;
 ...
 <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-[0_15px_40px_rgba(36,121,194,0.18)] border border-blue-50 p-4 md:col-span-2 flex flex-col h-[75vh] animate-fade-in">
        ...
        <div className="flex-1 border border-blue-100 rounded-2xl bg-gradient-to-br from-blue-50/80 via-white to-blue-100/70 p-3 overflow-y-auto text-xs md:text-sm">
          {messages.length === 0 ? (
            <p className="text-gray-400 text-xs">
              No messages yet. Start the conversation with admin/vendor.
            </p>
          ) : (
            messages.map((msg) => {
        ...
        {isChatClosed ? (
          <p className="mt-2 text-[11px] text-red-500">
            ...
            order. You can still see your conversation above.
          </p>
        ) : (
          <form
          ...
          >
            <input
              ...
              placeholder="Type your message to admin/vendor..."
  ...
  return (
    ...
    >
      <div className="w-full max-w-6xl mx-auto mt-14 mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 px-4">
        {/* Left: orders list (now includes completed + cancelled) */}
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-[0_15px_40px_rgba(36,121,194,0.18)] border border-blue-50 p-4 md:col-span-1">
          ...
          {activeOrders.length === 0 ? (
            <p className="text-sm text-gray-500 mt-4">
              You don&apos;t have any orders to chat about yet. Once you place
              an order, you can talk to the admin here.
            </p>
          ) : (
            <>
              <div className="space-y-3">
                {paginatedOrders.map((order) => {
                  ...
                  return (
                    <div
                      key={order._id}
                      className={`w-full text-left p-3 rounded-2xl border text-xs md:text-sm transition transform ${
                        isActive
