...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Convert orders into flat notification list (latest status per order)
      const notifs = orders
        .map((order) => {
          const latestStatusEntry = (order.statusHistory || []).slice(-1)[0];
          const when =
            latestStatusEntry?.updatedAt || order.updatedAt || order.createdAt;
          return {
            _id: order._id,
      ...
      // NEW: reset to first page on refresh/load
      setCurrentPage(1);
    } catch (error) {
      ...
      <div className="w-full max-w-5xl mx-auto mt-14 mb-8 px-4">
        <div className="bg-white/85 backdrop-blur-md rounded-2xl shadow-[0_15px_40px_rgba(36,121,194,0.18)] border border-blue-50 p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  All recent updates related to your orders in one place.
                </p>
              </div>
            </div>
            <button
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs bg-blue-50 text-blue-600`}
                            >
                              {meta.icon}
                            </div>
                            <div>
          ...
          <p className="mt-3 text-[11px] text-gray-500">
            For full details and chat related to an order, check{" "}
